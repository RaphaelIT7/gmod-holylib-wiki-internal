<?php
	...
	class Extension extends Parsedown
	{
			...
			if (preg_match('/<function name="([^"]+)" parent="([^"]*)" type="([^"]+)">([\s\S]*?)<\/function>/s', $text, $matches)) {
				if (isset($fullName))
				{
					if($matches[3] == 'classfunc' && $this->config['code_language'] == 'lua') {
						...
						$title = (strlen($matches[2]) > 0 ? ($matches[2] . '.') : '') . $matches[1];
					} else if($this->config['code_language'] == 'c++' && ($matches[3] == 'define' || $matches[3] == 'macro')) {
						$title = (strlen($matches[2]) > 0 ? ($matches[2] . '.') : '') . $matches[1];
					} else {
		...
		function GetTags($text)
		{
			...
			if (preg_match('/<function name="([^"]+)" parent="([^"]*)" type="([^"]+)">([\s\S]*?)<\/function>/s', $text, $matches)) {
				if (isset($matches[3]) && $matches[3] != '') {
					...
					if ($matches[3] == 'libraryfield' || $matches[3] == 'define' || $matches[3] == 'macro') {
						$tags .= 'cm meth memb';
					}
		...
		{
			if (isset($func['type']) && $func['type'] == 'define') {
				return '#define ' . $func['name'];
			}
			...
			$html = '<div class="function ' . $func['type'] . ' ' . $func['realm'] . '">';
				$html .= '<div class="function_line">';
					...
					if (sizeof($func['args']) != 0 || sizeof($func['rets']) != 0) {
						...
						if ($func['type'] == 'libraryfield' || $func['type'] == 'define' || $func['type'] == 'macro') {
							$html .= ' ' . $this->getFunctionName($func) . (isset($func['value']) ? (" = " . $func['value']) : '');
						} else {
		...
		protected function buildType($type)
		{
			$html = '<div class="type">';
				...
				#	$html .= $this->text($type['summ']);
				#$html .= '</div>';
				if ($type['is'] == 'convar' || $type['is'] == 'config' || $type['is'] == 'define')
				{
					$html .= '</div>';
					return $html;
				}
